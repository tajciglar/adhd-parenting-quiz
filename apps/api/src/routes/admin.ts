...
export default async function adminRoutes(fastify: FastifyInstance) {
  ...
  // ── GET /api/admin/version-dropoff ──────────────────────────────────────
  // V1 = before Mar 26 (childName was basic-info step 5–6, shown at start)
  // V2 = from Mar 26  (childName moved to processing screen popup)
  fastify.get("/admin/version-dropoff", async (request, reply) => {
    const { version } = request.query as { version?: string };
    ...
    try {
      ...
      const sortedSteps = Array.from(stepSessions.entries())
        .map(([step, sessions]) => ({ step, views: sessions.size }))
        .sort((a, b) => a.step - b.step);
