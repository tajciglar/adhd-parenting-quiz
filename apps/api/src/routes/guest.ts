...
export default async function guestRoutes(fastify: FastifyInstance) {
  ...
  // ── POST /api/guest/pdf ─────────────────────────────────────────────────────
  // Accepts a rendered report from the web client and returns the PDF on demand.
  const postPdfBodySchema = z.object({
    report: z.object({ archetypeId: z.string(), title: z.string(), innerVoiceQuote: z.string() }).passthrough(),
  ...
  const trackBodySchema = z.object({
    ...
    eventType: z.enum(["step_viewed", "quiz_completed", "checkout_started", "purchase_completed", "answer_submitted", "optin_completed", "optin_thankyou", "wp_checkout_redirect"]),
